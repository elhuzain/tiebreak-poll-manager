...
export function PollResultOptionRow({ option, ballots, tiedTop, isMine }: {
  option: Option; ballots: number; tiedTop: boolean; isMine: boolean;
}) {
  ...
  return <li className={`border-b-2 border-dashed border-cocoa/20 p-4 last:border-b-0 sm:p-5 ${tiedTop ? "bg-butter/40" : ""}`}>
    ...
    <div aria-hidden="true" className="mt-3 h-2.5 overflow-hidden rounded-full bg-cream-deep"><div className="h-full rounded-full bg-teal-deep" style={{ width: `${percent}%` }} /></div>
  </li>;
}
