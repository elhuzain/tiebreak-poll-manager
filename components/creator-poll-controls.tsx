...
export function CreatorPollControls({ slug, status }: { slug: string; status: "open" | "settled" }) {
  ...
  return <section aria-labelledby="share-title" className="mt-8 rounded-lg border-[2.5px] border-cocoa bg-card p-5 sm:p-7">
    ...
    <dialog ref={dialog} onClose={() => trigger.current?.focus()} aria-labelledby="poll-action-title" className="m-auto w-[calc(100%-2rem)] max-w-md rounded-lg border-[2.5px] border-cocoa bg-card p-6 text-cocoa backdrop:bg-cocoa/60">
      ...
      <div className="mt-6 flex flex-wrap gap-3"><button type="button" disabled={busy} onClick={() => dialog.current?.close()} className="min-h-11 rounded-full border-2 border-cocoa px-5 font-display text-sm font-extrabold">Cancel</button><button type="button" disabled={busy} onClick={() => void submit()} className="min-h-11 rounded-full border-2 border-teal-deep bg-teal-deep px-5 font-display text-sm font-extrabold text-cream-bright disabled:opacity-50">{busy ? "Please wait…" : status === "open" ? "End voting" : "Re-open voting"}</button></div>
    </dialog>
  </section>;
}
